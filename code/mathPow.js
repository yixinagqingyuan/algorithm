...
function pow4(base, exponent) {
  ...
  return Math.exp(exponent * Math.log(Math.abs(base))) * (base < 0 && exponent % 2 !== 0 ? -1 : 1);
}
...
function myPow(base, exponent) {
  ...
  if (!isFinite(base)) {
    if (Math.abs(base) === Infinity) {
      if (exponent > 0) return base > 0 ? Infinity : (exponent % 2 === 0 ? Infinity : -Infinity);
      if (exponent < 0) return 0;
    }
  }
...
function testPowImplementations() {
  ...
  console.log('格式: base^exponent = 原生结果 | 实现1 | 实现2 | 实现3 | 完整实现');
  console.log('=' .repeat(80));
  
  testCases.forEach(([base, exponent]) => {
    const original = Math.pow(base, exponent);
    const result1 = Number.isInteger(exponent) && exponent >= 0 ? pow1(base, exponent) : 'N/A';
    ...
    console.log(`${base}^${exponent} = ${original} | ${result1} | ${result2} | ${result3} | ${result5}`);
  });
}
