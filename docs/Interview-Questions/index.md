...
2、JS：沙箱（Proxy/快照/with），防止污染 window 其实从本质上说，js 沙箱要解决的就是一个问题全局window 的污染
...
type  和 interface 区别
interface 更适合"面向对象"和"可扩展"的场景，type 更适合"类型计算"和"复杂类型表达"的场景。 比如一些交叉类型，联合类型的声明 功能有重叠的部分
...
close事件关掉比如socket ,并且结束当前事件循环， 然后开始下次循环， 但当时走到 poll 就会停在这，等下一个事件来，
本质上也不是停在这，就是他事件循环结束了，等待下一次循环，因为大部分都是 io 的一些回调，所以当还有一些 io 操作的时候，会继续等在这，前提是，后面后面有任务，前面也有任务， 注意，他是先检查前面的任务，在检查后面的任务，如果没有就结束了，只要按照人正常的思维方式来设计，就能想明白
...
当然这也得按照实际情况来，因为我们是 cdn 在线链接，所以不能通过服务端下发不通负载机器做灰度，但是比如我们有的项目是通过域名访问的， 就可以配置代理，来确定灰度比例
...
这个问题我之前从头设计过，需求文档先行，要给整体的业务需求和整体的业务逻辑画一个流程出来，注意这里指的是大的项目，或者从头开始的项目，小的项目，比如重做个页面啥的可能不需要这么复杂，因为我也做过大项目和小项目，我觉得区别就是，小项目不需要做架构分层，上来直接怼上去就可以，大项目，比我我们这种灵动项目，还有云组件，还有监控平台这种项目，架构设计显得尤为重要

我举个例子，比如我们web ied 编辑器这块，由于复杂，所以bug比较多，功能也特别多，所以迭代优化特别多， 比如组件查询列表 基本做好了就二年不动，所以为了不影响整体项目运行的稳定，他必须通过 learn 的方式去管理，模块化的管理，这样就需要 通过架构分层的方式划分，不论是 ddd 这种领域驱动来通过业务划分，还是你通过改动功能模块来划分，都可以
接下来就是整体的架构设计了，服务需要哪些表，设计哪些servers，前端，怎么区分，比如我们当时是按照编辑器模块，是一个单独的模块 列表模块详情模块是一个单独的模块，但是后来随着项目的发展，有别的项目要单独接入我们的渲染器做渲染，所有后来我们有重新划分了整体的架构，渲染器单拎出来了，独立开发，但这样就会产生一个问题就是我每次开发编辑器这部分就得开俩项目，所以，有利有弊吧，看你怎么取舍
最后我们内部就会产生一些技术这计文档，这就是所谓的架构师做的事情，我们这内部。就比较 open 吧，反正每个人都有机会当架构师，看你有没有意愿去做，云组件，和小程序监控平台都是我争取来的

然后就是任务分配，任务协作的一些事情

接下来就是代码测试等等吧，然后就是一些收尾工作

然后就是这些好了就要开始做一些后续的数据统计，性能监控接入，埋点设计，等等，数据监控报表等等


## 狗东面试题

unknown 和 any 的区别
any：完全跳过类型检查，类型不安全，不推荐使用
unknown：强制类型检查，类型安全，是 any 的更好替代 也就是声明的时候无所谓，但是用的时候必须校验一下
vue setup 里头能获取到哪些金溪，不能获取到哪些信息

可以获取到的数据
1、provide依赖注入的数据
2、props父组件传过来的初始值
3、attrs 不在 props种声明的信息
4、emit声明
5、expose 暴露给父组件的方法
6、路由信息